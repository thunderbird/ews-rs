    ...
    B: OperationResponse,
{
    ...
    pub fn from_xml_document(document: &[u8]) -> Result<Self, Error> {
        ...
        match envelope.body {
            ...
            Err(fault) => Err(Error::RequestFault(Box::new(fault))),
        }
    }
}
...
mod tests {
    ...
    use crate::{
        types::common::{MessageXmlElement, MessageXmlElements, MessageXmlTagged, MessageXmlValue},
        types::sealed::EnvelopeBodyContents,
        Error, MessageXml, OperationResponse, ResponseCode, ServerBusy,
    };
